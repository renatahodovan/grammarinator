...
names in trees generated by a hypothetical code generator. In this tree,
...
Both scripts accept one or more transformers specified using the
``--transformer`` argument in the form of a fully qualified name.
...
The C++ backend accepts one or more transformer functions. They can be compiled
into the standalone generator and the AFL++ or libFuzzer integration using the
repeatable ``--transformer`` option of
:ref:`the C++ build script <cpp_compilation>`. Their declarations and
implementations must be made available with ``--include``.
The transformers run after a tree is generated or mutated and before the tree
is serialized into the target input.
