...
class ParserTool:
    ...
    def __init__(
        ...
        transformers: list[Callable[[Rule], Rule]] | None = None,
        max_depth: int | float = RuleSize.max.depth,
        strict: bool = False,
        ...
        lexer_root, parser_root = ProcessorTool.parse_grammars(
            grammars, parser_dir, encoding, errors, lib_dir
        )
        self._graph = ProcessorTool.build_graph(
            False, lexer_root, parser_root, None, rule
        )
        ...
        self._lexer_cls, self._parser_cls, self._listener_cls = self._build_grammars(
            grammars, self._parser_dir, antlr, lib_dir
        )
    ...
    @staticmethod
    def _build_grammars(
        in_files: list[str], out: str, antlr: str, lib_dir: str | None = None
    ) -> tuple[type[Lexer], type[Parser], type[ParseTreeListener]]:
        """
        ...
        try:
            ...
            try:
                run(
                    ("java", "-jar", antlr, languages["python"]["antlr_arg"])
                    + (("-lib", lib_dir) if lib_dir else ())
                    + grammars,
                    stdout=PIPE,
                    stderr=PIPE,
                    cwd=out,
                ...
                return splitext(
                    split(
                        list(
                            filter(
                                lambda x: (
                                    len(commonprefix([filename, x])) > 0
                                    and x.endswith(end_pattern)
                                ),
                                files,
                            )
                        )[0]
            ...
            listener = file_endswith(
                f"{languages['python']['listener_format']}.{languages['python']['ext']}"
            )
            ...
            return tuple(
                getattr(__import__(x, globals(), locals(), [x], 0), x)
                for x in [lexer, parser, listener]
            )
    ...
    def _antlr_to_grammarinator_tree(self, antlr_node, parser, visited=None):
        ...
        if isinstance(antlr_node, ParserRuleContext):
            ...
            if (
                class_name.endswith("Context")
                and class_name.lower() != rule_name.lower() + "context"
            ):
                alt_name = class_name[: -len("Context")]
                labeled_alt_node = UnparserRule(
                    name=(rule_name, alt_name[0].upper() + alt_name[1:])
                )
            ...
            for antlr_child in antlr_node.children or []:
                child, child_depth, child_rules = self._antlr_to_grammarinator_tree(
                    antlr_child, parser, visited
                )
                if not child:
                    continue
            ...
            assert isinstance(antlr_node, TerminalNode), (
                f"An ANTLR node must either be a ParserRuleContext or a TerminalNode but {antlr_node.__class__.__name__} was found."
            )
            name, text = (
                parser.symbolicNames[antlr_node.symbol.type]
                if len(parser.symbolicNames) > antlr_node.symbol.type
                else "<INVALID>",
                antlr_node.symbol.text,
            )
            assert name, f"{name} is None or empty"

            if (
                antlr_node.symbol.type == Token.EOF
            ...
            if not self._hidden:
                node = UnlexerRule(
                    name=(name,),
                    src=text,
                    immutable=(name,) in self._graph.immutables or name == "<INVALID>",
                )
                rules.add(node)
            else:
                node = []
                hidden_tokens_to_left = (
                    parser.getTokenStream().getHiddenTokensToLeft(
                        antlr_node.symbol.tokenIndex, -1
                    )
                    or []
                )
                        ...
                        if token not in visited:
                            ...
                            node.append(
                                UnlexerRule(
                                    name=hidden_name,
                                    src=token.text,
                                    immutable=hidden_name in self._graph.immutables,
                                )
                            )
                            visited.add(token)
                ...
                node.append(
                    UnlexerRule(
                        name=(name,),
                        src=text,
                        immutable=(name,) in self._graph.immutables
                        or name == "<INVALID>",
                    )
                )
                hidden_tokens_to_right = (
                    parser.getTokenStream().getHiddenTokensToRight(
                        antlr_node.symbol.tokenIndex, -1
                    )
                    or []
                )
    ...
    def _adjust_tree_to_generator(
        self, rules: Iterable[UnlexerRule | UnparserRule]
    ) -> None:
        ...
        def _adjust_rule(rule):
            def _match_seq(grammar_vertices, tree_node_pos):
                ...
                for vertex_pos, vertex in enumerate(grammar_vertices):
                    if vertex is None:  # end-of-rule marker
                        return seq_children if tree_node_pos == len(
                            tree_nodes
                        ) else None, tree_node_pos
                    ...
                    if isinstance(vertex, UnparserRuleNode):
                        if (
                            tree_node_pos < len(tree_nodes)
                            and isinstance(tree_nodes[tree_node_pos], UnparserRule)
                    ...
                    if isinstance(vertex, UnlexerRuleNode):
                        if (
                            tree_node_pos < len(tree_nodes)
                            and isinstance(tree_nodes[tree_node_pos], UnlexerRule)
                            and (
                                vertex.name == "_".join(tree_nodes[tree_node_pos].name)
                                or tree_nodes[tree_node_pos].name == ("<INVALID>",)
                                and vertex.name.startswith("T__")
                                and tree_nodes[tree_node_pos].src
                                == vertex.out_neighbours[0].src
                            )
                        ):
                            seq_children += [tree_nodes[tree_node_pos]]
                            tree_node_pos += 1
                    ...
                    if isinstance(vertex, AlternationNode):
                        for alternative_vertex in vertex.out_neighbours:
                            assert isinstance(alternative_vertex, AlternativeNode), (
                                alternative_vertex
                            )
                            ...
                            if (
                                len(out_neighbours) == 1
                                and isinstance(out_neighbours[0], UnparserRuleNode)
                                and len(out_neighbours[0].id) == 3
                                and out_neighbours[0].name == "_".join(vertex.rule_id)
                            ):
                                out_neighbours = out_neighbours[0].out_neighbours
                            alt_children, alt_tree_node_pos = _match_seq(
                                out_neighbours, tree_node_pos
                            )
                            if alt_children is not None:
                                rest_children, rest_tree_node_pos = _match_seq(
                                    grammar_vertices[vertex_pos + 1 :],
                                    alt_tree_node_pos,
                                )
                                if rest_children is not None:
                                    return seq_children + [
                                        (
                                            UnparserRuleAlternative(
                                                alt_idx=alternative_vertex.alt_idx,
                                                idx=alternative_vertex.idx,
                                            ),
                                            alt_children,
                                        )
                                    ] + rest_children, rest_tree_node_pos
                        return None, tree_node_pos
                    ...
                    if isinstance(vertex, QuantifierNode):
                        ...
                        for _ in range(0, int(vertex.start)):
                            quant_children, quant_tree_node_pos = _match_seq(
                                vertex.out_neighbours, tree_node_pos
                            )
                            if quant_children is None:
                                return None, tree_node_pos
                            quantifier_children += [
                                (UnparserRuleQuantified(), quant_children)
                            ]
                        ...
                        backtrack_positions = []
                        for _ in (
                            range(int(vertex.start), int(vertex.stop))
                            if vertex.stop != inf
                            ...
                            if quant_children is None:
                                rest_children, rest_tree_node_pos = _match_seq(
                                    grammar_vertices[vertex_pos + 1 :], tree_node_pos
                                )
                                if rest_children is not None:
                                    return seq_children + [
                                        (
                                ...
                                return (
                                    None,
                                    tree_node_pos,
                                )  # Complete failure, return the last unmatched position
            ...
            for child in rule.children:
                if (
                    isinstance(child, UnlexerRule)
                    and "_".join(child.name) in self._hidden
                ):
            ...
            rule_children, rule_tree_node_pos = _match_seq(
                self._graph.vertices[rule.name].out_neighbours + [None], 0
            )
            if rule_children is None:
                logger.warning(
                    "Failed to match %s tree node to the related grammar rule at %d.",
                    rule.name,
                    rule_tree_node_pos,
                )
                return
            ...
            for child, prev_child in hidden_nodes:
                if prev_child is None:
                    ...
                    prev_child.parent.insert_child(
                        prev_child.parent.children.index(prev_child) + 1, child
                    )
        ...
        for rule in rules:
            ...
            for child in rule.children:
                if (
                    isinstance(child, UnparserRuleAlternative)
                    and len(child.children) == 1
                    and isinstance(grandchild := child.children[0], UnparserRule)
                    and len(grandchild.children) == 1
                    and isinstance(
                        grandgrandchild := grandchild.children[0],
                        UnparserRuleAlternative,
                    )
                    and len(rule.name) == 1
                    and len(grandchild.name) == 2
                    and rule.name[0] == grandchild.name[0]
    ...
    def _create_tree(self, input_stream: InputStream, fn: str | None) -> Rule | None:
        try:
            ...
            if parser._syntaxErrors:
                if self._strict:
                    logger.warning(
                        "%s syntax errors detected in %s. Skipping.",
                        parser._syntaxErrors,
                        fn,
                    )
                    return None
                logger.warning(
                    "%s syntax errors detected in %s.", parser._syntaxErrors, fn
                )

            root, depth, rules = self._antlr_to_grammarinator_tree(
                parse_tree_root, parser
            )
            assert root and isinstance(root, UnparserRule), (
                "The root of the converted tree is None or not a parser rule."
            )
            if depth > self._max_depth:
                logger.warning(
                    "The tree representation of %s is %s, too deep. Skipping.",
                    fn,
                    depth,
                )
                return None
    ...
    def parse(self, fn: str) -> None:
        ...
        try:
            root = self._create_tree(
                FileStream(fn, encoding=self._encoding, errors=self._errors), fn
            )
