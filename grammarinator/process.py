...
class FuzzerGenerator(object):
    ...
    def generate_header(self, grammar_name, fuzzer_type, options, combined):
        ...
        src += self.line('from grammarinator.model import *')
        ...
        with self.indent():
            src += self.line('def __init__(self, {args}):'.format(args='*, model=None, max_depth=float(\'inf\'), weights=None, cooldown=1.0' if unlexer else 'unlexer'))
            ...
            with self.indent():
                ...
                if unlexer:
                    src += self.line('self.model = model or RandomModel()')
    ...
    def generate_single(self, node, parent_id):
        ...
        if isinstance(node, (self.antlr_parser_cls.RuleAltListContext, self.antlr_parser_cls.AltListContext, self.antlr_parser_cls.LexerAltListContext)):
            ...
            result = self.line('choice = self.unlexer.model.choice([0 if {{{alt_name}}}[i] > self.unlexer.max_depth else w * self.unlexer.weights.get(({alt_name!r}, i), 1) for i, w in enumerate([{weights}])])'
        ...
        if isinstance(node, (self.antlr_parser_cls.ElementContext, self.antlr_parser_cls.LexerElementContext)):
            ...
            with self.indent():
                result += self.line('for _ in self.unlexer.model.{quant_type}():'.format(quant_type=quant_type))
